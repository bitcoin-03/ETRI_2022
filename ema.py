...
class EMA:
    ...
    def __init__(
        ...
        assert 0.0 <= decay_rate <= 1.0, "Decay rate should be in range [0, 1]"
    ...
    def update(self, parameters: Iterable[torch.nn.Parameter]) -> None:
        ...
        if self.num_updates is not None:
            decay_rate = min(
                self.decay_rate, (1 + self.num_updates) / (10 + self.num_updates)
            )
        else:
    ...
    def store(self, parameters: Iterable[torch.nn.Parameter]) -> None:
        ...
        """
        parameters = list(parameters)
        self.saved_params = [p.clone().detach() for p in parameters if p.requires_grad]
    ...
    def copy_back(self, parameters: Iterable[torch.nn.Parameter]) -> None:
        ...
        if self.saved_params is None:
            raise ValueError("No saved parameters found.")
    ...
    def to(self, device="cpu", dtype=None) -> None:
        ...
        if self.saved_params is not None:
            self.saved_params = [
                p.to(device=device, dtype=dtype) for p in self.saved_params
            ]
    ...
    def load_state_dict(self, state_dict: dict) -> None:
        ...
        assert 0.0 <= self.decay_rate <= 1.0, "Decay rate should be in range [0, 1]"
