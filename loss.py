...
class LADELoss(nn.Module):
    ...
    def forward(self, y_pred, target, q_pred=None):
        ...
        per_cls_pred_spread = y_pred.T * (
            target == torch.arange(0, self.classes).view(-1, 1).type_as(target)
        )  # C x N
        pred_spread = (
            y_pred
        ...
        num_samples_per_cls = torch.sum(
            target == torch.arange(0, self.classes).view(-1, 1).type_as(target), -1
        ).float()  # C
...
def calculate_prior(classes, img_max=None, prior=None, prior_txt=None, reverse=False, return_num=False):
    if prior_txt:
        labels = []
        ...
        img_num_per_cls = [occur_dict[i] for i in range(classes)]
    else:
        img_num_per_cls = []
        for cls_idx in range(classes):
            if reverse:
                num = img_max * (prior ** ((classes - 1 - cls_idx) / (classes - 1.0)))
            else:
                num = img_max * (prior ** (cls_idx / (classes - 1.0)))
